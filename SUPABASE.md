...
Если Supabase не настроен, кнопка «Войти» показывает сообщение «Авторизация временно недоступна», а новости берутся из демо-массива в `news.js`.
...
Откройте файл `config.js` и впишите значения:
...
- `config.js` — единственное место, где хранятся ключи.
- `auth.js` — модуль `VSRF_AUTH`: инициализирует клиент, обрабатывает вход/выход, показывает/скрывает `.admin-only` элементы, публикует состояние.
- `news.js` — модуль `VSRF_NEWS.fetchNews(limit)`: сначала пробует Supabase, при ошибке / отсутствии — возвращает демо-массив.
